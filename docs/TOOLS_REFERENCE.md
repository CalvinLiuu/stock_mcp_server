...
## 📊 All Available Tools (48+ functions)
...
### 📊 Market Sentiment Tracking (8)
| Tool | Description |
|------|-------------|
| `get_market_sentiment()` | Overall market sentiment score (-100 to +100) |
| `get_detailed_sentiment_signals()` | Detailed breakdown of all signals |
| `get_vix_analysis()` | VIX volatility analysis (fear/greed) |
| `get_market_breadth()` | Sector participation analysis |
| `get_sector_rotation_signal()` | Defensive vs growth rotation |
| `get_ai_sector_signal()` | AI/Tech sector strength analysis |
...
### For Market Timing & Sentiment Analysis
- `get_market_sentiment()` - Overall market direction
- `get_vix_analysis()` - Fear/greed gauge
- `get_sector_rotation_signal()` - Risk-on/risk-off
- `get_ai_sector_signal()` - Tech leadership status
- `analyze_leverage_indicators()` - Market stress signals
- `track_sentiment_history()` - Trend analysis
...
track_sentiment_history(days=30)
```
